...
新增單字採自由工作區形式，整合手寫、文字輸入、AI 討論與 AI 生成預覽。
...
3. AI 討論延伸  
   AI 在討論中提出易混淆單字、相似意思或詞性變化。使用者確認後可建立新字卡與關聯。

重複判斷：

- 系統以 `normalizedWord` 與 `partOfSpeech` 判斷單字是否已存在（同一單字文字但詞性不同視為不同 Card，不套用以下重複規則，直接視為新字卡建立流程）。
- 使用者主動嘗試建立已存在單字（含詞性皆相同）時，該 Card 的 `repeatCount +1`。
...
建立 sense（無論 AI 生成或手動新增）時，中文意思、使用情境、例句、例句中文翻譯皆為必填，於建立當下就需完整填寫，不採事後補上的方式。備註為選填，可事後新增或編輯。
...
- 一筆關聯連結的是雙方字卡各自「特定」的 sense（見第 4 章 `sourceSenseId`／`targetSenseId`），而非整張字卡；同一張字卡若有多個 sense，不同 sense 可分別建立各自的關聯。
...
| `usageContext` | 使用情境或語意說明；建立時必填，初始由 AI 產生，使用者可手動編輯。 |
...
| `sourceCardId` | 來源字卡。 |
| `targetCardId` | 目標字卡。 |
| `sourceSenseId` | 來源字卡中，此關聯實際配對到的特定 sense。 |
| `targetSenseId` | 目標字卡中，此關聯實際配對到的特定 sense。 |
...
- 詞性、音標於單字詳情僅顯示、不可編輯；中文意思、使用情境、例句、例句中文翻譯與備註可於 sense 手動編輯。
